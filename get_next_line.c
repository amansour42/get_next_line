...
void    ft_build(char **str, char *s)
{
    ...
    s2 = NULL;
    if ((s2 = (char*)malloc(sizeof(char) * (ft_strlen(*str) + ft_strlen(s) + 1))))
    {
        if (**str)
            ft_strcpy(s2, *str);
...
int	    get_next_line(const int fd, char **line)
{
    ...
    if (!line || fd < 0 || !(current = build_strcuture(fd, &get))
               || !(current->str = (char*)malloc(BUFF_SIZE + 1)))
        return (-1);
    *line = (current->s) ? ft_strdup(current->s) : "\0";
    if (get_line(line, &(current->s)) == 1)
        return (1);
    while (((i = read(fd, current->str, BUFF_SIZE)) == BUFF_SIZE) &&
            (!(ft_memchr((void*)(current->str), '\n', BUFF_SIZE))))
        ft_build(line, current->str);
    if (i <= 0 && !(*line)[0])
        return (i);
    (current->str)[i] = '\0';
    current->s = ft_strdup(current->str);
    ft_build(line, current->str); 
    free(current->str);
